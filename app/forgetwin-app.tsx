...
import { componentCatalog, demoComponentIds } from '../lib/forge-data';
import { sensorX, useForge, useForgeWebMCP } from '../lib/use-forge';
import type { ForgeState, ForgeToolName, Metrics, ToolResult } from '../lib/forge-types';
...
export function ForgeTwinApp() {
  ...
  const call = async (name: ForgeToolName, input: Record<string, unknown> = {}, delay = 105) => {
    const result = must(command(name, input, 'UI'));
    await pause(delay);
    return result;
  };
  ...
  const runInitialBuild = async () => {
    ...
    try {
      ...
      await call('set_design_goal', { throughput_bpm: 20, min_accuracy_pct: 95, max_components: 7 }, 160);
      for (const catalogId of demoComponentIds) await call('add_component', { catalog_id: catalogId }, 125);
      ...
      await call('set_motor_speed', { component_id: 'conveyor-main', speed_mps: 2 }, 90);
      await call('set_actuator_timing', { actuator_id: 'diverter-servo', delay_ms: 1040, hold_ms: 520 }, 120);
      const run = await runMachine('UI');
      must(run);
      setToast('First trial failed in physics — collision evidence captured');
    } catch (caught) {
  ...
  const diagnoseAndFix = async () => {
    ...
    try {
      ...
      await call('set_actuator_timing', { actuator_id: 'diverter-servo', delay_ms: failedRun.recommendedDelayMs, hold_ms: 520 }, 200);
      must(await runMachine('UI'));
      setToast('Design passes — now move the sensor to challenge the agent');
    } catch (caught) {
  ...
  const retuneHumanEdit = async () => {
    ...
    try {
      ...
      await call('set_actuator_timing', { actuator_id: 'diverter-servo', delay_ms: failedRun.recommendedDelayMs, hold_ms: 520 }, 200);
      must(await runMachine('UI'));
      const finalSensorX = sensorX(getSnapshot());
  ...
  const runHeaderSimulation = async () => {
    if (!state.components.length) return runInitialBuild();
    if (busy) return;
    setBusy(true);
    setError(null);
  ...
  const addOrSelect = (catalogId: string) => {
  ...
  if (state.screen === 'landing') return <Landing state={state} toolCount={registeredTools} onEnter={() => patchUi({ screen: 'lab' })} onDemo={runInitialBuild} />;
  ...
  return (
    <div className="forge-shell">
      ...
      <header className="forge-header">
        <button className="brand-lockup" aria-label="ForgeTwin home" onClick={() => patchUi({ screen: 'landing' })}>
        ...
        <div className="header-actions">
          <button className="ghost-button" onClick={() => { checkpoint('Manual design checkpoint'); setToast('Revision checkpoint saved'); }}><Save size={14} />Save</button>
          <button className="ghost-button" onClick={undo}><Undo2 size={14} />Undo</button>
          <button className="ghost-button" onClick={() => setDrawer('compare')}><GitCompareArrows size={14} />Compare</button>
          <button className="ghost-button" onClick={() => { reset('lab'); setToast('Deterministic demo reset'); }}><RotateCcw size={14} />Reset</button>
      ...
      <main id="forge-main" className="forge-main">
        <aside className="catalog-panel" aria-label="Component catalog">
          ...
          <div className="catalog-list">
            {componentCatalog.slice(0, 7).map((item, index) => {
              ...
              return <button className={`catalog-card ${placed ? 'placed' : ''}`} key={item.catalogId} onClick={() => addOrSelect(item.catalogId)} aria-pressed={placed}>
          ...
          {selected?.id === 'sensor-color' && <SensorInspector state={state} onMove={handleSensorMove} />}
        </aside>
        ...
        <section className="viewport-panel" aria-label="3D machine workspace">
          ...
          {!state.components.length && <div className="empty-machine-card"><span className="goal-avatar"><Sparkles size={18} /></span><span className="eyebrow">Agent-ready workspace</span><h1>Give the AI a goal.<br />Watch it engineer.</h1><p>Every part, connection, simulation, failure, and redesign is a visible WebMCP action on shared state.</p><button className="run-button large" onClick={runInitialBuild} disabled={busy}><Play size={15} fill="currentColor" />Build & run first design</button></div>}
          ...
          {humanChallenge && <div className="challenge-banner"><span className="challenge-icon"><MoveHorizontal size={18} /></span><div><span className="eyebrow">Human + agent challenge</span><strong>Drag the cyan sensor upstream.</strong><p>The agent must detect your shared-state change and retune without moving it back.</p></div><button onClick={() => { patchUi({ selectedComponentId: 'sensor-color', xray: true }); setToast('Drag the cyan bridge in 3D, or use its keyboard controls'); }}>Select sensor</button></div>}
        ...
        <aside className="agent-panel" aria-label="Agent activity">
          ...
          <MetricStack metrics={latestRun?.metrics ?? null} phase={state.phase} />
        </aside>
      </main>
...
function Landing({ state, toolCount, onEnter, onDemo }: { state: ForgeState; toolCount: number; onEnter: () => void; onDemo: () => void }) {
  return <div className="landing-shell">
    ...
    <main className="landing-hero">
      <div className="hero-copy"><span className="hero-kicker"><Sparkles size={13} />Agent-native digital twin engineering</span><h1>Don’t generate it.<br /><em>Engineer it.</em></h1><p>ForgeTwin gives AI agents a real machine workspace: components, constraints, shared state, and a physics world where bad designs fail.</p><div className="hero-actions"><button className="run-button hero" onClick={onDemo}><Play size={15} fill="currentColor" />Watch the AI engineer</button><button className="ghost-button hero-secondary" onClick={onEnter}><Code2 size={15} />Explore the lab</button></div><div className="hero-proof"><span><strong>18</strong> scoped WebMCP tools</span><span><strong>60 Hz</strong> fixed physics</span><span><strong>7</strong> component limit</span></div></div>
      <div className="hero-machine"><ForgeScene state={state} preview onSensorMove={() => undefined} onSelect={() => undefined} /><div className="hero-hud top"><span>LIVE DIGITAL TWIN</span><strong>COLOR SORTER / REV 03</strong></div><div className="hero-hud bottom"><span>THROUGHPUT TARGET</span><strong>20+ BOXES / MIN</strong></div><div className="hero-orbit-label one"><i />Sensor beam</div><div className="hero-orbit-label two"><i />Servo path</div></div>
    </main>
