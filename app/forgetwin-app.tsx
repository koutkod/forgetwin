...
export function ForgeTwinApp() {
  ...
  return <div className="forge-shell"><a className="skip-link" href="#forge-main">Skip to engineering workspace</a>
    ...
    <main id="forge-main" className="forge-main">
      ...
      <aside className="agent-panel" aria-label="Agent activity"><div className="side-tabs"><button className={sideTab === 'chat' ? 'active' : ''} onClick={() => setSideTab('chat')}><MessageSquareText size={13} />Chat edit</button><button className={sideTab === 'activity' ? 'active' : ''} onClick={() => setSideTab('activity')}><Activity size={13} />Activity</button><button className={sideTab === 'history' ? 'active' : ''} onClick={() => setSideTab('history')}><History size={13} />History</button></div>{sideTab === 'chat' ? <AgentChat messages={editMessages} prompt={editPrompt} busy={busy} model={agentModel} modelConnected={agentRuntime === 'server-model' || agentRuntime === 'session-model'} selected={selected} onPromptChange={setEditPrompt} onSubmit={editWithChat} /> : sideTab === 'activity' ? <AgentFeed state={state} toolCount={registeredTools} trace={agentTrace} runtime={agentRuntime} model={agentModel} busy={busy} canCancel={agentCancelable} onCancel={cancelAgentRun} onConfigure={() => setAgentSettingsOpen(true)} /> : <RevisionHistory state={state} onRestore={(revision) => { const result = command('restore_revision', { revision }, 'UI'); if (result.ok) setToast(`Revision ${revision} restored`); else setError(result.error.message); }} />}<MetricStack metrics={latestRun?.designHash === state.designHash ? latestRun.metrics : null} phase={latestRun?.designHash === state.designHash ? state.phase : state.components.length ? 'ready' : state.phase} />{state.goal && <p className="model-note"><AlertTriangle size={12} />{state.goal.disclaimer}</p>}</aside>
    </main>
...
function AgentChat({ messages, prompt, busy, model, modelConnected, selected, onPromptChange, onSubmit }: { messages: EditMessage[]; prompt: string; busy: boolean; model: string; modelConnected: boolean; selected: MachineComponent | null; onPromptChange: (value: string) => void; onSubmit: (value: string) => void }) {
  const examples = selected
    ? [`Make ${selected.role} 20% larger`, `Move ${selected.role} up 0.5 m`, `Use aluminum for ${selected.role}`]
    : ['Make the base wider', 'Lengthen the boom', 'Add another support'];
  return <div className="agent-chat"><div className="panel-heading"><div><span className="eyebrow">Natural-language world revision</span><h2>Edit with chat</h2></div><span className={`agent-live ${modelConnected ? 'model' : 'local'}`}><i />{modelConnected ? model : 'LOCAL'}</span></div>
    ...
    <form className="chat-composer" onSubmit={(event) => { event.preventDefault(); onSubmit(prompt); }}><label className="sr-only" htmlFor="machine-edit-prompt">Describe a change to the current machine</label><textarea id="machine-edit-prompt" value={prompt} onChange={(event) => onPromptChange(event.target.value)} placeholder="e.g. Widen the crane base and move the counterweight backward" maxLength={300} rows={3} disabled={busy} /><button type="submit" disabled={busy || prompt.trim().length < 3} aria-label="Apply chat edit">{busy ? <Cpu size={14} /> : <Send size={14} />}{busy ? 'Engineering…' : 'Apply & simulate'}</button></form>
