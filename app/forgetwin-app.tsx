...
export function ForgeTwinApp() {
  ...
  return <div className="forge-shell"><a className="skip-link" href="#forge-main">Skip to engineering workspace</a>
    <header className="forge-header">
      ...
      <div className="header-center"><span className={`live-dot ${agentRuntime === 'session-model' ? 'cyan' : ''}`} />{agentRuntime === 'session-model' ? `${agentModel} connected with your key` : 'Local engineer ready'} <span className="header-divider" /> REV {state.revision.toString().padStart(2, '0')} <span className="header-divider" /> {registeredTools === FORGE_TOOL_COUNT ? `${registeredTools} WebMCP tools live` : 'WebMCP host not connected'}</div>
    ...
    <main id="forge-main" className="forge-main">
      <aside className="catalog-panel" aria-label="World hierarchy">
        ...
        {selected && <ComponentInspector component={selected} state={state} busy={busy} onMove={(position) => handleEditableMove(selected.id, position)} onUpdate={manualUpdate} />}
      </aside>
...
function ComponentInspector({ component, state, onMove, onUpdate, busy }: { component: MachineComponent; state: ForgeState; onMove: (position: [number, number, number]) => void; onUpdate: (name: ForgeToolName, input: Record<string, unknown>, message: string) => void; busy: boolean }) {
