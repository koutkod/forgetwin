...
- Joint limits encode physical travel, not a schema placeholder. Use limits: null for continuous revolute wheels, shafts, gears, rollers, and rotors. Use a finite [min,max] pair for bounded hinges and for every prismatic, spring, or rope joint. Couple each drive to the exact joint it moves.
...
- Use limits: null for continuous revolute motion and a finite [min,max] pair for bounded hinges, prismatic travel, springs, and ropes. When changing a body transform, preserve or coherently update its mounts, driven joint, sensors, and control relationships.
...
- target_ids are the bodies intentionally changed or created. A transform of a jointed body also retargets its mount, so include the opposite joint endpoint in target_ids and never put that endpoint in preserve_ids. preserve_ids should explicitly cover nearby or user-protected bodies whose placement/function must remain unchanged. verification must state observable checks tied to the request.`
